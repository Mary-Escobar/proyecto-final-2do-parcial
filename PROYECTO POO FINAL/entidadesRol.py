...
class Empresa:
    ...
    def getEmpresa(self):
        return  [self.ruc,self.razonSocial,self.direccion,self.telefono]
...
class Administrativo(Empleado):
    ...
    def getEmpleado(self):
        return  [str(self.id),self.nombre,str(self.departamento.id),str(self.cargo.id),self.direccion,self.cedula,self.telefono,str(self.fechaIngreso),str(self.sueldo),str(self.comision)]
...
class Obrero(Empleado):
    ...
    def valorAntiguedad(self):
        return round(int(self.fechaIngreso.split('-')[0])/365*self.sueldo,2)  
    
    def getEmpleado(self):
        return  [str(self.id),self.nombre,str(self.departamento.id),str(self.cargo.id),self.direccion,self.cedula,self.telefono,str(self.fechaIngreso),str(self.sueldo),str(self.cc)]
...
class Prestamo:
    def __init__(self,empleado, aamm, valor, numPagos,saldo, estado= True,id=1):
        ...
        self.cuota = round(valor/numPagos)
        self.saldo = saldo
        self.estado = estado
...
class Nomina:
    ...
    def mostrarCabeceraNomina(self,razonSocial, direccion, telefono, ruc,tipoRol):
        borrarPantalla()
        print(f'{Fore.YELLOW}              {razonSocial} Ruc : {ruc} Teléfono : {telefono} Dirección: {direccion}')
        print(f'{Fore.RED}--------------------------------------------------------------------------------------------------------------------')
        print(f'{Fore.BLUE}FECHA: {Fore.WHITE}{self.fecha}  {Fore.YELLOW} N O M I N A   D E   P A G O  D E   E M P LE A D O S: {Fore.CYAN} {tipoRol}  ')
        print(f'{Fore.BLUE}Nomina correspondiente al Periodo:{Fore.WHITE}{self.aamm}{Fore.RED}')
        print('--'*59) 
        print(f'{Fore.MAGENTA}'," "*5,"E M P L E A D O S"," "*34,"I N G R E S O S"," "*30,"E G R E S O S")
        print(f"{Fore.GREEN}Nombre     Cargo          Departamento    Sueldo   Sobretiempo  Antiguedad  Comision TotIng   Iess    Prestamo   TotDes   Neto{Fore.WHITE}")
...
class DetalleNomima(CalculoRol):
    ...
    def getSobretiempo(self,aamm):
      ...
      if self.empleado.id[0]=="O":
        archiPrestamo = Archivo("./archivos/sobretiempo.txt","|")
        sobretiempo = archiPrestamo.buscar2(self.empleado.id,aamm)
        if sobretiempo:
            entsobretiempo = Sobretiempo(self.empleado,sobretiempo[2],int(sobretiempo[3]),int(sobretiempo[4]),sobretiempo[5],sobretiempo[0])
            calSob = entsobretiempo.sobretiempo()
      return calSob
    ...
    def getAntiguedad(self,deduccion):
        ...
        if self.empleado.id[0] =="O":
            if deduccion:
                calAnt = round(deduccion.getAntiguedad()* self.empleado.valorAntiguedad(),2)
        return calAnt
